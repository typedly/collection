 ...
 * @extends {CollectionShape<C, E, T, R>} The main collection functionalities defined in `CollectionShape`.
...
export interface AdaptableCollectionShape<
  A extends CollectionAdapter<C, E, T, R>,
...
> extends CollectionShape<C, E, T, R>, AdaptableDataShape<A, T, C, R> {
   ...
   * @template {C} NC 
   * @template {CollectionAdapter<NC, E, T, R>} NA 
   * @param {NC} settings The new configuration settings for the collection.
   * @returns {AdaptableCollectionShape<NA, NC, E, T, R>} 
   */
  with?<NC extends C, NA extends CollectionAdapter<NC, E, T, R>>(settings: NC): AdaptableCollectionShape<NA, NC, E, T, R>;
}
